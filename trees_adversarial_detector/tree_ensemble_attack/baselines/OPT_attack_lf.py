...
class OPT_attack_lf(object):
    def __init__(self, model, norm_order):
        self.model = model
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.01, iterations = 1000):
        ...
        for i in range(num_directions):
            ...
            try:
                ...
                if np.argmax(model.predict_label(x0+theta))!=y0:
                    #l2norm = self.norm(theta)
                    initial_lbd = self.norm(theta.flatten())
                    theta /= initial_lbd     # might have problem on the defination of direction
                    lbd, count = self.fine_grained_binary_search(model, x0, y0, theta, initial_lbd, g_theta)
    ...
    def fine_grained_binary_search_local(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        try:
            ...
            if np.argmax(model.predict_label(x0 + lbd * theta)) == y0:
                lbd_lo = lbd
                lbd_hi = lbd * 1.01
                nquery += 1
                while np.argmax(model.predict_label(x0 + lbd_hi * theta)) == y0:
                    lbd_hi = lbd_hi * 1.01
                    nquery += 1
                    if lbd_hi > 20:
                        return 99999, nquery
                ...
                lbd_lo = lbd * 0.99
                nquery += 1
                while np.argmax(model.predict_label(x0 + lbd_lo * theta)) != y0:
                    lbd_lo = lbd_lo * 0.99
                    nquery += 1
            ...
            while (lbd_hi - lbd_lo) > tol:
                lbd_mid = (lbd_lo + lbd_hi) / 2.0
                nquery += 1
                if np.argmax(model.predict_label(x0 + lbd_mid * theta)) != y0:
                    lbd_hi = lbd_mid
                else:
                    lbd_lo = lbd_mid
            return lbd_hi, nquery
    ...
    def fine_grained_binary_search(self, model, x0, y0, theta, initial_lbd, current_best):
        try:
            ...
            nquery = 0
            if initial_lbd > current_best:
                if np.argmax(model.predict_label(x0+current_best*theta)) == y0:
                    nquery += 1
                    return float('inf'), nquery
                lbd = current_best
            else:
    ...
    def fine_grained_binary_search_local_targeted(self, model, x0, y0, t, theta, initial_lbd= 1.0, tol=1e-5):
        try:
            ...
            if np.argmax(model.predict_label(x0 + lbd * theta)) != t:
                lbd_lo = lbd
                lbd_hi = lbd * 1.01
                nquery += 1
                while np.argmax(model.predict_label(x0 + lbd_hi * theta)) != t:
                    lbd_hi = lbd_hi * 1.01
                    nquery += 1
                    if lbd_hi > 100:
                        return float('inf'), nquery, 1.0
                ...
                lbd_lo = lbd * 0.99
                nquery += 1
                while np.argmax(model.predict_label(x0 + lbd_lo * theta)) == t:
                    lbd_lo = lbd_lo * 0.99
                    nquery += 1
            ...
            while (lbd_hi - lbd_lo) > tol:
                lbd_mid = (lbd_lo + lbd_hi) / 2.0
                nquery += 1
                if np.argmax(model.predict_label(x0 + lbd_mid * theta)) == t:
                    lbd_hi = lbd_mid
                else:
                    lbd_lo = lbd_mid
            temp_theta = np.abs(lbd_hi * theta)
    ...
    def fine_grained_binary_search_targeted(self, model, x0, y0, t, theta, initial_lbd, current_best):
        try:
            ...
            nquery = 0
            if initial_lbd > current_best:
                if np.argmax(model.predict_label(x0 + current_best * theta)) != t:
                    nquery += 1
                    return float('inf'), nquery
                lbd = current_best
            else:
